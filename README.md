...
문서와 예제 중심의 작은 기여부터 시작해, 실제 사용자들이 헷갈리기 쉬운 동작을 더 명확하게 설명하는 데 집중했습니다.  
현재는 Airflow와 Kafka 생태계의 사용 경험을 바탕으로 documentation contribution을 이어가고 있으며, 이후에는 테스트·버그 수정·core 코드 레벨의 기여까지 확장하는 것을 목표로 하고 있습니다.

| Project | PR | Contribution |
| --- | --- | --- |
| Apache Airflow | [#66597](https://github.com/apache/airflow/pull/66597) | Clarified `HttpOperator.response_filter` and XCom usage in the documentation and system test example. Updated the example to match the actual `httpbin /get` response structure. |
...
| kafka-python | [#3015](https://github.com/dpkp/kafka-python/pull/3015) | Added a `KafkaConsumer` manual offset commit example and clarified that consumers should commit the next offset to consume, e.g. `message.offset + 1`. |

- 3 upstream PRs merged into Apache Airflow and kafka-python.
- Focused on documentation accuracy, runnable examples, and developer experience for data workflow/Kafka users.
- Next goal: move beyond documentation into tests, bug fixes, and core implementation contributions.
